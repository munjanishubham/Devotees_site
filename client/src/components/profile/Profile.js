...
const Profile = ({
    getProfileById,
    profile: {profileById, loading}, authPandit,authUser, match
}) => {
    useEffect(() => {
                                ...
                                <div class="current-profile">
                                    <div class="current-btn">
                                        ...
                                        {authUser.isUserAuthenticated ? (
                                            <Fragment>
                                                <Link to={`/appointment/${profileById.pandit._id}`} type="button" className="rounded-pill btn btn-info"><i className="fas fa-calendar-check"></i> Book Appointment</Link>
                                            </Fragment>
                                            ) : (
                                                <Fragment>
                                        ...
                                        {authPandit.isPanditAuthenticated && 
                                            authPandit.loadingPandit === false && 
                                            authPandit.pandit._id === profileById.pandit._id && (
                                    ...
                                    <div class="devotee-review">
                                            ...
                                            }
                                            {
                                                profileById.review.map(rev => (
                                                    <ProfileReview key={rev._id} review={rev} panditId={match.params.id} />
                                                ))
                                            }
                                        </div>
                                </div>
