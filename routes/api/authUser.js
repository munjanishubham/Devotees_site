...
router.delete("/:appointment_id", authUser, async (req, res) => {
  try {
    ...
    const panditId = user.appointments.map((item) =>
      item.id === req.params.appointment_id ? item.pandit : null
    );

    const newId = panditId.filter((pandit) => pandit !== null);

    const profile = await Profile.findOne({ pandit: newId[0] });
